...
- **Improved Filtering**: Fixed location and level filtering to work with real PocketBase data
- **Dynamic Level Detection**: Levels are now dynamically determined from actual schedule data
...
- Fixed schedule filtering logic for locations and levels
...
### 🎨 **Color Theme System & UI Improvements**
- **New Color Theme System**: Added `color_themes` collection in PocketBase for dynamic category colors
- **Enhanced News Categories**: Updated `news_categories` to use color themes instead of hardcoded colors
- **Improved Category Display**: Fixed uppercase forcing issue - categories now display as entered in database
- **Dynamic Color Support**: Categories can now use custom colors with transparency settings from PocketBase

### 🥊 **Punching Bag Section Fixes**
- **Fixed Animation Bug**: Eliminated duplicate star appearance after punch animation completion
- **Restored Promo Code Functionality**: Fixed promo code copying to "Additional Information" field in contact form
- **Enhanced User Experience**: Added smooth scroll and focus to contact form field after promo code generation
- **Improved State Management**: Connected PunchingBagSection and ContactSection through MainRenderer state

### 🔝 **New Scroll-to-Top Button**
- **JAB-Style Design**: Added floating scroll-to-top button with red gradient matching site theme
- **Smart Visibility**: Button appears after scrolling 300px down the page
- **Smooth Animations**: Hover effects, scaling, and pulsing animations
- **Accessibility**: Proper ARIA labels and keyboard navigation support
- **Responsive Design**: Works perfectly on all device sizes

### 📚 **Documentation Updates**
- **PocketBase Collections**: Added comprehensive documentation for `color_themes` collection
- **Setup Guide**: Updated collection setup instructions with new color theme system
- **Examples**: Added sample data for color themes and updated category examples
- **Backup System**: Enhanced backup documentation with latest changes

### 🔧 **Technical Improvements**
- **TypeScript Interfaces**: Added `ColorTheme` interface and updated `NewsCategory` interface
- **PocketBase Integration**: Enhanced expand parameters to load color theme data
- **State Management**: Improved component communication between sections
- **Error Handling**: Better error handling for color theme fallbacks

### 🐛 **Bug Fixes**
- Fixed transparency logic for color themes (0 = fully opaque, 100 = fully transparent)
- Fixed falsy value handling for transparency settings
- Fixed category color fallback system
- Fixed promo code state synchronization between components

---

## v1.1.0 - 2025-09-05

### 🎯 **Trainer Modal & UI Improvements**
- **Redesigned Trainer Modal**: Modern vertical layout with photo on top, information below
- **Enhanced Modal Background**: Multi-layered gradients, blur effects, and animated decorative elements
- **Improved Modal Scrolling**: Fixed content cutoff issues, proper internal scrolling
- **Smart Close Button**: Always visible close button that scrolls with content without creating layout gaps
- **Hidden Scrollbar**: Clean modal appearance with hidden scrollbar for better UX

### 🔧 **Technical Fixes**
- **Fixed PocketBase API**: Updated deprecated `pb.files.getUrl()` to `pb.files.getURL()`
- **Fixed Image Warnings**: Added proper aspect ratio handling for Next.js Image components
- **Fixed AbortController Error**: Resolved "autocancelled" error in trainer data fetching
- **Fixed CSS @import Order**: Corrected CSS import rules to prevent browser warnings
- **Fixed useEffect Dependencies**: Resolved React Strict Mode double-invocation issues

### 🎨 **UI/UX Enhancements**
- **Clickable Trainer Cards**: Removed "Подробнее" button, made entire cards clickable
- **Improved Preloader Button**: Better positioning and styling for "Пропустить" button
- **Disabled Smooth Scrolling**: Replaced with native browser scrolling for better performance
- **Enhanced Typography**: Proper rich text rendering from PocketBase with Tailwind Typography

### 📁 **Project Structure**
- **Created Tailwind Config**: Proper configuration for typography plugin
- **Updated Dependencies**: All packages up to date
- **Backup System**: Comprehensive backup in both project and user directories

### 🐛 **Bug Fixes**
- Fixed modal opening causing page scroll to top
- Fixed anchor link positioning issues
- Fixed console warnings and errors
- Fixed preloader hanging issue with timeout fallback
